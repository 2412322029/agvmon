    ...
    <n-card title="异常记录管理" :bordered="false">
      ...
      <n-modal v-model:show="showPreviewModal" preset="card" style="width: 800px;" title="导出文本预览">
        <n-card title="预览内容" size="small" embedded>
          <pre
            style="white-space: pre-wrap; word-break: break-all; max-height: 400px; overflow-y: auto; margin: 0; padding: 10px; border-radius: 4px;">{{ previewContent }}</pre>
        </n-card>
        <template #footer>
          <n-space justify="end">
...
<script setup>
import { NButton, NCard, NDataTable, NDatePicker, NForm, NFormItem, NInput, NModal, NPopconfirm, NPopover, NSpace, NSwitch, NText, NUpload, useMessage } from 'naive-ui'
...
const fetchData = async () => {
  ...
  try {
    ...
    if (result.message === 'success') {
      ...
      paginationProps.itemCount = result.data.total_count
    } else {
